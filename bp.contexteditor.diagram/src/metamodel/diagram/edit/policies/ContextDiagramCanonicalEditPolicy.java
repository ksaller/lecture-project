...
import metamodel.diagram.part.MetamodelDiagramUpdater;
import metamodel.diagram.part.MetamodelLinkDescriptor;
import metamodel.diagram.part.MetamodelNodeDescriptor;
...
public class ContextDiagramCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected List getSemanticChildrenList() {
		...
		List<MetamodelNodeDescriptor> childDescriptors = MetamodelDiagramUpdater
				.getContextDiagram_1000SemanticChildren(viewObject);
		for (MetamodelNodeDescriptor d : childDescriptors) {
	...
	protected void refreshSemantic() {
		...
		for (Iterator<MetamodelNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			MetamodelNodeDescriptor next = descriptorsIterator.next();
			String hint = MetamodelVisualIDRegistry.getType(next.getVisualID());
		...
		for (MetamodelNodeDescriptor next : childDescriptors) {
			String hint = MetamodelVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<MetamodelLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = MetamodelVisualIDRegistry
			...
			for (Iterator<MetamodelLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				MetamodelLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<MetamodelLinkDescriptor> collectAllLinks(View view,
			Domain2Notation domain2NotationMap) {
		if (!ContextDiagramEditPart.MODEL_ID.equals(MetamodelVisualIDRegistry
				.getModelID(view))) {
	...
	private Collection<IAdaptable> createConnections(
			Collection<MetamodelLinkDescriptor> linkDescriptors,
			Domain2Notation domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (MetamodelLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					MetamodelVisualIDRegistry.getType(nextLinkDescriptor
	...
	protected final EditPart getHintedEditPart(EObject domainModelElement,
		...
		View view = (View) domain2NotationMap.getHinted(domainModelElement,
				MetamodelVisualIDRegistry.getType(hintVisualId));
		if (view != null) {
			return (EditPart) getHost().getViewer().getEditPartRegistry()
					.get(view);
		}
