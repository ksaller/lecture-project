...
import contextmapper.diagram.edit.commands.AssociateConnectionCreateCommand;
import contextmapper.diagram.edit.commands.AssociateConnectionReorientCommand;
...
public class ContextItemSemanticEditPolicy extends
	...
	public ContextItemSemanticEditPolicy() {
		super(ContextmapperElementTypes.Context_2001);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (ContextmapperVisualIDRegistry.getVisualID(incomingLink) == AssociateConnectionEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
						incomingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
